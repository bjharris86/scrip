...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\mvt.aprx")
...
denslayer = thegdb + "\\d_"+ mv
denslayer1 = thegdb + "\\d1_"+ mv
...
for lyt in aprx.listLayouts():
    ...
    if layout_extent:    
        ...
        fullpath = os.path.join(outputfolder,mv)
        print("local datastore: "+fullpath)
        sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
        fullsql= os.path.join(sqlpath,mv)
        print("sql datastore"+ ""+sqlpath)
        subfolder_path = os.path.join(outputfolder, mv)
        if os.path.exists(subfolder_path):
            shutil.rmtree(subfolder_path)
            os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
        output_path = os.path.join(outputfolder,mv, f"{lyt.name}.png")
