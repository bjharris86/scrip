...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\Guns_MVT.aprx")
...
guns = m.listLayers("Guns Stolen YTD")[0]
gs = "guns_stolen"
...
outkdens = KernelDensity(guns,"NONE","25","5280","SQUARE_MILES","DENSITIES","PLANAR")
denslayer = thegdb + "\\d_"+ gs
denslayer1 = thegdb + "\\d1_"+ gs
...
for lyt in aprx.listLayouts():
    ...
    if layout_extent:    
        ...
        fullpath = os.path.join(outputfolder,gs)
        print("local datastore: "+fullpath)
        sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
        fullsql= os.path.join(sqlpath,gs)
        print("sql datastore"+ ""+sqlpath)
        subfolder_path = os.path.join(outputfolder, gs)
        if os.path.exists(subfolder_path):
            shutil.rmtree(subfolder_path)
            os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
        output_path = os.path.join(outputfolder,gs, f"{lyt.name}.png")
