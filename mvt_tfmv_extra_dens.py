...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\tfmv_mvt_extra.aprx")
...
mvt = m.listLayers("prv mvt_tfmv")[0]
word = "TFMV_MVT_extra_patrol"
...
outkdens = KernelDensity(mvt,"NONE","25","5280","SQUARE_MILES","DENSITIES","PLANAR")
denslayer = thegdb + "\\d_"+ word
denslayer1 = thegdb + "\\d1_"+ word
...
for lyt in aprx.listLayouts():
    ...
    if layout_extent:    
        ...
        fullpath = os.path.join(outputfolder,word)
        print("local datastore: "+fullpath)
        sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
        fullsql= os.path.join(sqlpath,word)
        print("sql datastore"+ ""+sqlpath)
        subfolder_path = os.path.join(outputfolder, word)
        if os.path.exists(subfolder_path):
            shutil.rmtree(subfolder_path)
            os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
        output_path = os.path.join(outputfolder,word, f"{lyt.name}.png")
