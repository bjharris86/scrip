...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\vcd_agg_firearm.aprx")
...
gun = m.listLayers("YTD Agg Firearm")[0]
agg = "vcd_firearm_agg_assault"
...
outkdens = KernelDensity(gun,"NONE","25","5280","SQUARE_MILES","DENSITIES","PLANAR")
denslayer = thegdb + "\\d_"+ agg
denslayer1 = thegdb + "\\d1_"+ agg
...
for lyt in aprx.listLayouts():
    ...
    if layout_extent:    
        ...
        fullpath = os.path.join(outputfolder,agg)
        print("local datastore: "+fullpath)
        sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
        fullsql= os.path.join(sqlpath,agg)
        print("sql datastore"+ ""+sqlpath)
        subfolder_path = os.path.join(outputfolder, agg)
        if os.path.exists(subfolder_path):
            shutil.rmtree(subfolder_path)
            os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
        output_path = os.path.join(outputfolder,agg, f"{lyt.name}.png")
