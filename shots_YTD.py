...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\shots.aprx")
...
for val in vallist: 
    ...
    try:
        ...
        pass
    arcpy.management.MakeXYEventLayer(sqltable,"i_xcoord","i_ycoord",templayer, theprojection,"")
    # --- ONE MILE --- 
    ...
    for lyt in aprx.listLayouts():
        ...
        if layout_extent:    
            ...
            fullpath = os.path.join(outputfolder,val)
            print("local datastore: "+fullpath)
            sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
            fullsql= os.path.join(sqlpath,val)
            print("sql datastore"+ ""+sqlpath)
            subfolder_path = os.path.join(outputfolder, val)
            if os.path.exists(subfolder_path):
                shutil.rmtree(subfolder_path)
                os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
            output_path = os.path.join(outputfolder,val, f"{lyt.name}.png")
