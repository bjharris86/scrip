...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\cppi.aprx")
...
cfs = m.listLayers("CFS Previous week")[0]
ep = "CPPI_extra_patrol"
...
outkdens = KernelDensity(cfs,"NONE","25","5280","SQUARE_MILES","DENSITIES","PLANAR")
denslayer = thegdb + "\\d_"+ ep
denslayer1 = thegdb + "\\d1_"+ ep
...
for lyt in aprx.listLayouts():
    ...
    if layout_extent:    
        ...
        fullpath = os.path.join(outputfolder,ep)
        print("local datastore: "+fullpath)
        sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
        fullsql= os.path.join(sqlpath,ep)
        print("sql datastore"+ ""+sqlpath)
        subfolder_path = os.path.join(outputfolder, ep)
        if os.path.exists(subfolder_path):
            shutil.rmtree(subfolder_path)
            os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
        output_path = os.path.join(outputfolder,ep, f"{lyt.name}.png")
