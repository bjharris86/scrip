...
aprx = arcpy.mp.ArcGISProject(r"C:\Users\bjharris\Desktop\Maps\st_rob.aprx")
...
rob = m.listLayers("Street Rob YTD")[0]
sr = "street_robbery"
...
outkdens = KernelDensity(rob,"NONE","25","5280","SQUARE_MILES","DENSITIES","PLANAR")
denslayer = thegdb + "\\d_"+ sr
denslayer1 = thegdb + "\\d1_"+ sr
...
for lyt in aprx.listLayouts():
    ...
    if layout_extent:    
        ...
        fullpath = os.path.join(outputfolder,sr)
        print("local datastore: "+fullpath)
        sqlpath = '\\\\pdrmscvpr16\\sdd_images\\automated_maps\\' 
        fullsql= os.path.join(sqlpath,sr)
        print("sql datastore"+ ""+sqlpath)
        subfolder_path = os.path.join(outputfolder, sr)
        if os.path.exists(subfolder_path):
            shutil.rmtree(subfolder_path)
            os.makedirs(subfolder_path)
            # Export layout as PNG, overwriting existing files
        output_path = os.path.join(outputfolder,sr, f"{lyt.name}.png")
